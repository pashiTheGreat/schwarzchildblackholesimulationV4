// Historical 2D visualization target.  It is not a Schwarzschild validation
// reference; the active CPU physics owner is physics/schwarzschild.cpp.
#include <GL/glew.h>
...
struct BlackHole {
    ...
    void draw() {
        ...
        for (int i = 0; i <= 100; i++) {
            const float angle = static_cast<float>(2.0 * M_PI * i / 100.0);
            const float x = static_cast<float>(r_s * cos(angle));
            const float y = static_cast<float>(r_s * sin(angle));
            glVertex2f(x, y);
        }
        glEnd();
    }
...
struct Ray {
    ...
    void draw(const std::vector<Ray>& rays) {
        ...
        for (const auto& ray : rays) {
            glVertex2f(static_cast<GLfloat>(ray.x), static_cast<GLfloat>(ray.y));
        }
        glEnd();
